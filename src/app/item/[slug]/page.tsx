...
export default async function ItemPage({ params }: Props) {
  ...
  const detailRows = [
    ...
    ["品牌", item.brand],
    ["型号", item.model],
    item.original_purchase_price && item.original_purchase_price > 0 && item.current_retail_price && item.current_retail_price > 0
      ? ["原购买价", formatPublicPrice(item.original_purchase_price, item.currency)]
      : null
  ...
  return (
    <main className="mx-auto grid max-w-4xl gap-6 px-4 py-5 sm:px-6">
          ...
          {referencePrice !== null && (
            <div>
              ...
              <p className="mt-1 text-lg font-bold text-ink/60 line-through">
                {formatPublicPrice(referencePrice, item.currency)}
              </p>
            </div>
          )}
      ...
      <section className="grid gap-5 pb-12">
        ...
        {(detailRows.length > 0 || item.reference_url) && (
